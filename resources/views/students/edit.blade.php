...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-xl font-bold text-slate-900 tracking-tight">Form Edit Siswa</h3>
            <p class="text-sm text-slate-500 mt-1">Perbarui data siswa dengan benar.</p>
        </div>
        ...
        <form action="{{ route('students.update', $student->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                <label for="photo" class="block text-sm font-semibold text-slate-800 mb-1.5">
                    Foto Siswa <span class="text-xs font-normal text-slate-500">(Kosongkan jika tidak ingin mengubah / Format JPG &amp; PNG. Jika &gt; 100KB akan otomatis dikompres)</span>
                </label>
                ...
                <div class="mt-2 flex items-center gap-4">
                    <div id="photo-preview-container" class="w-20 h-20 rounded-2xl bg-slate-100 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden flex-shrink-0">
                        @if($student->photo)
                            ...
                            <svg id="photo-placeholder" class="w-8 h-8 text-slate-400 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
